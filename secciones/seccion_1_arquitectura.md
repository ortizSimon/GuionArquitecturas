...
| **D2 — Usuarios y Cuentas** | Punto 1 (persona natural o jurídica) + 1a/1b: consulta unificada de cuentas, carga masiva de bancos, sincronización diaria |
...
| **Eventos que consume** | `SuspiciousPatternDetected` (desde D8) |
...
**Responsabilidad:** Registrar y mantener actualizada la información de personas naturales y jurídicas (empresas) junto con sus cuentas bancarias en bancos filiales.
| Aspecto | Detalle |
|----------|----------|
| **Actores** | Persona natural, empresa aliada, bancos filiales (proveedores de datos), administrador del sistema |
| **Funciones clave** | Carga masiva desde bancos, sincronización diaria de información de clientes, consulta de estado de cuentas registradas (personas naturales y jurídicas), gestión de cuentas bancarias de empresas aliadas |
| **Eventos que produce** | `UserRegistered`, `AccountLinked`, `AccountSyncCompleted` |
| **Eventos que consume** | — |
| **RNF dominante** | Fiabilidad, Consistencia eventual |
| **Observaciones** | El registro de usuarios NO es autoservicio; es un proceso ETL programado. La sincronización diaria debe ser idempotente para tolerar reintentos. Los saldos de cuentas se consultan en tiempo real a los bancos filiales vía D6 (Integraciones); D2 no almacena saldos localmente, solo la referencia y metadata de las cuentas. Esto elimina la necesidad de consumir eventos de D4 para actualizar saldos. |
| **Consideraciones asignadas** | 1. Gestión multibanco — consulta unificada para persona natural o jurídica (Primario)<br>16. Volumen masivo de usuarios (~25M) (Primario)<br>17. Registro de usuarios mediante carga masiva (Primario)<br>18. Sincronización diaria de clientes (Primario) |
| **Comunicación con otros dominios** | Responsabilidad: usuarios, cuentas bancarias afiliadas (personas naturales y jurídicas), estados de cuenta agregados, sincronización masiva.<br><br>Se comunica con:<br>- D1 IAM (síncrono): validar identidad/permiso<br>- D3 Empresas y Empleados (síncrono): resolver la asociación empresa → cuentas bancarias registradas (cuenta origen para nómina)<br>- D6 Integraciones (síncrono): consulta de saldos en tiempo real a bancos filiales; (asíncrono o batch): sincronizar cuentas con bancos afiliados, procesos de carga masiva<br>- D4 Transferencias (síncrono): validar cuentas origen/destino (existencia, estado, límites)<br>- D5 Billetera (síncrono): mostrar saldo combinado / mover fondos desde/hacia billetera (según diseño)<br>- D8 Auditoría (asíncrono): registrar eventos de consulta crítica, cambios en perfil, carga/sync completada |
...
| **Comunicación con otros dominios** | Responsabilidad: empresas aliadas, empleados (solo referencia mínima), endpoints para obtener datos del empleado desde la empresa.<br><br>Se comunica con:<br>- D2 Usuarios y Cuentas (síncrono): asociar empresa con sus cuentas bancarias registradas (cuenta origen para pagos de nómina)<br>- D6 Integraciones (síncrono): “conector” hacia APIs de empresas para obtener datos de empleados<br>- D7 Pagos Masivos (síncrono): entregar lista de empleados activos / endpoints para resolver “empleado → destino de pago”<br>- D1 IAM (síncrono): control de acceso empresarial (admins, payroll managers)<br>- D8 Auditoría (asíncrono): registro de cambios empresariales, altas/bajas de empleados (referencias), resultados de consulta |
...
| Aspecto | Detalle |
|----------|----------|
| **Actores** | Persona natural (cuenta particular), Billetera Digital (D5), Empresa (vía D7 Pagos Masivos) |
| **Funciones clave** | Transferencia intra-banco (mismo banco filial), transferencia inmediata entre bancos filiales distintos, envío a ACH para bancos no filiales e internacionales, actualización inmediata del estado de cuenta en filiales, registro del movimiento tras resolución de ACH, validación en línea contra listas blancas/grises/negras antes de ejecutar la transferencia |
| **Eventos que produce** | `TransferInitiated`, `TransferCompletedImmediately`, `TransferSentToACH`, `TransferACHResolved`, `FraudCheckFlagged` |
| **Eventos que consume** | `ACHResponseReceived` (desde sistema externo ACH vía D6), `FraudListUpdated` (desde D8, para invalidar caché de listas antifraude) |
| **RNF dominante** | Consistencia (ACID en filiales), Disponibilidad, Tiempo de respuesta < 2 s |
| **Observaciones** | Patrón **Saga coreografiada** para coordinar pasos distribuidos (débito → crédito → confirmación). Compensación automática ante fallo parcial. |
...
| **Funciones clave** | Acreditar/debitar saldo de la billetera, mover dinero a cuentas activas del usuario, a cuentas externas (no filiales) y a cuentas internacionales (aplican las mismas reglas de D4), realizar pagos a terceros mediante pasarelas de pago |
...
| **Funciones clave** | Almacenar histórico inmutable de transacciones, generar y enviar extracto trimestral por usuario/banco, generar reporte semestral a la Superfinanciera, análisis post-hoc y correlación de transacciones sospechosas (Secundario; la validación en línea es responsabilidad de D4) |
| **Eventos que produce** | `SuspiciousPatternDetected`, `FraudListUpdated`, `QuarterlyStatementGenerated`, `SemiannualReportSent` |
...
| **Observaciones** | Usa un modelo **Event Sourcing** / append-only para el histórico. Realiza análisis post-hoc y correlación de patrones sospechosos (Kafka Streams / Flink); la validación en línea contra listas blancas/grises/negras ocurre en D4 antes de ejecutar cada transferencia. D8 retroalimenta las listas y detecta patrones históricos que D4 no puede identificar en tiempo real. |
...
| **Ítems transversales y ubicación recomendada** | 29. Detección de intrusiones → D8 (Primario), D1/D6 (Secundario).<br><br>36. Multiplataforma → Aplica a todos los dominios con interfaz de usuario: D1 (login), D2 (consulta de cuentas), D3 (gestión empresarial), D4 (transferencias), D5 (billetera), D7 (nómina).<br><br>37. Optimización gama media/alta → Transversal (presentación + rendimiento).<br><br>38. Pantalla mínima 6 pulgadas → Transversal (presentación).<br><br>39. UX consistente y agradable → Transversal (presentación).<br><br>40. Disponibilidad 24/7 → Transversal (SRE/Plataforma; si debes asignar: D8 Secundario, D7 Secundario).<br><br>41. Respuesta < 2 segundos → Transversal (si debes asignar: D4/D2/D7 según caso de uso).<br><br>42. Desempeño bajo picos → D7 (Primario), D4 (Secundario). |
